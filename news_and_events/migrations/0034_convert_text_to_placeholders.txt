...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for item in orm.Event.objects.all():
            old_field_content = item.content # the old field we want to convert
            if old_field_content and not getattr(item, "body", None):
                placeholder=Placeholder(slot="body")
                placeholder.save()
                ...
                plugin.body = old_field_content
                plugin.save()

                # set the old field to ""
                setattr(item, content, "")
